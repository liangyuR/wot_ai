...
def TestMinimapDetection(image_path: str, template_path: str) -> np.ndarray:
    ...
    anchor_detector = MinimapAnchorDetector(template_path=str(template_path), debug=False)
...
def main():
    ...
    model_path = script_dir / "best_m_detect.pt"
    if not model_path.exists():
        logger.error(f"错误：模型不存在: {model_path}")
        return False

    minimap_frame = TestMinimapDetection(minimap_path, minimap_template_path)
    success, results, minimap = TestYOLODetector(minimap_frame, model_path, confidence_threshold=0.5)
    if success:
        logger.info(f"检测结果: {results.raw_detections}")
    else:
        logger.error("检测失败")
        return False
